...
function scheduleNotification(reminder) {
  const logMessage = `[${new Date().toISOString()}] scheduling notification: ${JSON.stringify(
    reminder
  )}`;
  ...
  const timeoutId = setTimeout(() => {
    showNotification("Memory Fragment Alert", reminder.text, "info");
    scheduledReminders.delete(reminder.id);
...
function startBackgroundCronService() {
  const { forceStopAllServices } = require("./serviceManager");
...
function stopBackgroundCronService() {
  if (childProcess) {
    try {
      ...
      console.log(`[DEBUG] Error stopping child process: ${err.message}`);
    }
  }
